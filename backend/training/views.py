...
def complete_training_session(request, session_id):
    ...
    session.end_time = (
        ...
        else request.data.get("end_time")
    )
    session.is_completed = True
    session.completed_exercises = request.data.get(
        "completed_exercises", session.total_exercises
    )
    ...
    ai_result = call_deepseek_ai(
        session, duration_seconds, user_self_rating, user_feedback
    )
    ...
    try:
        ...
        for i in range(len(exercises) - 1):
            ...
            edge, _ = ExerciseGraph.objects.get_or_create(
                from_exercise=curr, to_exercise=next_ex
            )

            # 加权逻辑：完成度越高权重越大
            weight_add = 1
            if session.performance_score >= 4:
                weight_add = 2

            ExerciseGraph.objects.filter(pk=edge.pk).update(
                weight=F("weight") + weight_add
            )
...
class UserTrainingSessionListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return UserTrainingSession.objects.filter(user=self.request.user).order_by(
            "-start_time"
        )
...
def delete_training_exercise_record(request, record_id):
    ...
    record = get_object_or_404(
        UserTrainingExerciseRecord, id=record_id, session__user=user
    )

    # 检查会话是否已完成
    if record.session.is_completed:
        return Response(
            {"error": "训练会话已结束，无法删除记录"},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def user_training_stats(request):
    ...
    # 统计用户的训练数据
    total_sessions = UserTrainingSession.objects.filter(
        user=user, is_completed=True
    ).count()

    # 计算总训练时长
    sessions_with_duration = (
        UserTrainingSession.objects.filter(user=user, is_completed=True)
    ...
    total_calories = (
        UserTrainingSession.objects.filter(user=user, is_completed=True).aggregate(
            Sum("calories_burned")
    ...
    weekly_sessions = UserTrainingSession.objects.filter(
        user=user, is_completed=True, start_time__gte=week_ago
    ).count()
    ...
    stats = {
        ...
        "total_duration_formatted": str(
            timedelta(seconds=int(total_duration))
        ),  # 格式化时间
        "weekly_sessions": weekly_sessions,
        "total_calories": round(total_calories, 2),
        "best_performance_score": best_record.performance_score if best_record else 0,
        "favorite_plan": (
            best_record.plan.name if best_record and best_record.plan else ""
        ),
    }
...
def get_plan_days(request, plan_id):
    ...
    if not plan.is_public and plan.created_by != user:
        return Response(
            {"error": "无权访问此训练计划"}, status=status.HTTP_403_FORBIDDEN
        )
...
def generate_smart_plan(request):
    ...
    if UserTrainingSession.objects.filter(user=user).count() < 3:
        ...
        if cold_start_result:
            ...
            records = UserTrainingExerciseRecord.objects.filter(
                session=ref_session
            ).order_by("created_at")
